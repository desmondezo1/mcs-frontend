...
const Cart = () => {
  ...
  return (
    ...
    >
      {openCart === true ? (
        <div>
          <div className="bg-black fixed w-full h-full left-0 top-0 opacity-60 "></div>
          <div className="bg-[#F0F0F0] right-0 top-0 z-10 fixed w-[250px] bottom-0 flex flex-col ">
            <div className="py-3 top">
              ...
              {cartList.length > 0 ? (
                ...
                <div className="my-3">
                  <p className="text-center text-gray-500">Carrello vuoto</p>
                </div>
              )}
            </div>
            {cartList.length !== 0 && (
              <div className="mt-auto p-3 border-t border-gray-400 border-solid">
                ...
                <div className="buttons text-sm">
                  <Link href="/shop/cart">
                    <p
                      className="border-1 border-black border-solid rounded-3xl px-2 text-center py-1 my-2 cursor-pointer"
                      onClick={() => dispatch(updateCartVisibility(false))}
                    >
                      VISUALIZZA CARELLO
                    </p>
