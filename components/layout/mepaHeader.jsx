...
return (
    ...
    `}</style>
    <nav className={`${"navbar navbar-light navbar-expand-lg "} ${HeaderCss.MepaNavbar}`}>
    <div className="container">
        {openCart && <Cart />}
        <Link href="/">
            <a className={`${"navbar-brand navbarBrand"}`} >{(router.asPath == "/mepa")?
             ( <Image className={HeaderCss.MepaNavbarBrand} src={LogoWhite} alt="" height={"36.57px"} />) : (
                <Image className={HeaderCss.MepaNavbarBrand} src={Logo} alt="" height={"36.57px"} />
             )}
            </a> 
        </Link>
        ...
        <div className={`${"collapse navbar-collapse end-0"} ${HeaderCss.navListConatiner}`} id="navbarText">
            <ul className="navbar-nav ms-auto mb-2 mb-lg-0 ">
                <li className={` ${"nav-item"}`}>
                    <Link href="/shop">
                        <a className={`${"active"} ${"nav-link"}`} aria-current="page" style={{background: "#fff", color: "#1E3F7F!important"}} >SHOP</a>
                    </Link>
                </li>
                <li className={`${"nav-item"}`}>
                    <Link href="/#sect1">
                        <a className={`${router.asPath == "/#sect1" ? "active" : ""} ${"nav-link"}`}  style={{color: "#000"}}>SERVIZI</a>
                    </Link>
                </li>
                <li className={`${"nav-item"}`}>
                    <Link href="/#sect2">
                        <a className={`${router.asPath == "/#sect2" ? "active" : ""} ${"nav-link"}`} style={{color: "#000"}} >CHI SIAMO</a>
                    </Link>
                </li>
                <li className={`${"nav-item"}`}>
                    <Link href="/mepa">
                        <a className={`${router.asPath == "/mepa" ? "active" : ""} ${"nav-link"}`} style={{color: "#000"}}>MEPA</a>
                    </Link>
                </li>
                <li className={`${"nav-item"}`}>
                    <Link href="/#sect4">
                        <a  className={`${router.asPath == "/#sect4" ? "active" : ""} ${"nav-link"}`} style={{color: "#000"}} >CONTATTI</a>
                    </Link>
                </li>
                <li  className={`${"nav-item"}`}>
                    <Link href="/accedi-registrati">
                        <a className={`${router.asPath == "/accedi-registrati" ? "active" : ""} ${"nav-link"}`} style={{color: "#000"}} >ACCEDI/REGISTRATI</a>
                    </Link>
                </li>
            </ul>
            <div className={`${"icon-wrapper"} ${HeaderCss.iconWrapper}`} style={{color: "#000"}}>               
